...
/**
 * Gets a note from the database using the access link ID.
...
const handler = async (
  ...
  if (req.method === HttpMethods.POST) {
    ...
    const accessLinkId = req.query.accessLinkId as string;
    ...
    const note = await Note.getWithLinkId(accessLinkId, LinkIdFields.ACCESS);
    ...
    if (note.viewsLimitEnabled) {
      const viewsLimit = note.viewsLimit || 0;
      if (note.viewsCount >= viewsLimit) {
